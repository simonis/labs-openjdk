...
void ThreadLocalAllocBuffer::startup_initialization() {
  ...
  // NOTE (chaeubl): we don't need to reserve any extra space in the TLAB because
  // the prefetch instructions that we use are guaranteed to never trigger a segfault.
#ifdef COMPILER2
...
void ThreadLocalAllocStats::initialize() {
  ...
  if (UsePerfData) {
    ...
    _perf_max_refills             = data->max_fills();
    _perf_total_allocations       = data->alloc();
    _perf_total_gc_waste          = data->gc_waste();
    _perf_max_gc_waste            = data->max_gc_waste();
    _perf_total_refill_waste      = data->refill_waste();
    _perf_max_refill_waste        = data->max_refill_waste();
    _perf_total_slow_allocations  = data->slow_alloc();
    _perf_max_slow_allocations    = data->max_slow_alloc();
#else
    EXCEPTION_MARK;
