...
NOT_SVM(ExceptionsEventLog* Events::_exceptions = nullptr;)
...
void Events::init() {
  if (LogEvents) {
    ...
    NOT_SVM(_exceptions = new ExceptionsEventLog("Internal exceptions", "exc");)
...
void UnloadingEventLog::log(Thread* thread, InstanceKlass* ik) {
  ...
  SVM_ONLY(Unimplemented();) NOT_SVM(ik->name()->print_value_on(&st);)
}
